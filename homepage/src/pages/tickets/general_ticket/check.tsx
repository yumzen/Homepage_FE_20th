        ...
        <div className="font-['pretendard'] mx-[12.5vw] flex items-center flex-col mb-[84px] ">
                ...
                {merchant_order_id && (
                    <div className="mt-[12px] sm:mt-[48px] mx-auto items-center content-center flex flex-col ">
                        <Ticket_info reservation_id={Array.isArray(router.query.merchant_order_id) ? router.query.merchant_order_id.join('') : router.query.merchant_order_id || ''} buyer={buyer} />
                        <div className="flex mt-[16px] mx-auto sm:ml-[4px] flex-col w-[75vw] sm:w-[300px] md:w-[516px] text-left text-[#4A4A4A] text-[10px] sm:text-[12px] leading-[21px]">
                            <p> 예매 취소 인증 절차입니다.</p>
                            <p> 정말 취소하시려면 예매하실 때 입력한 전화번호를 입력해주세요.</p>
                            <input type="text" value={input_phone_num} onChange={(e) => set_Phone_num(e.target.value)} placeholder="전화번호를 입력해 주세요." className="mt-[10px] bg-[#FFF] border border-[#4A4A4A] rounded-[10px] text-[10px] sm:text-[14px] outline-none w-[150px] h-[28px] sm:w-[180px] sm:h-[36px] text-[black] px-[8px]" onKeyDown={handleInputKeyPress}/>
                        ...
                        <button onClick={handleCancelmerchant_order} className="w-full h-full bg-[#E8E8E8] rounded-[10px] text-center text-[#000] hover:text-[#FFF] text-[14px] font-[600] leading-[19px] hover:bg-[#281CFF]">
                            예매 취소하기
                        </button>
                        </div>
                    </div>
